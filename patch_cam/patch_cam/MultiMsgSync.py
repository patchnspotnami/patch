...
# maybe add in spatial data
        if "spatialData" not in self.msgs[seq]:
            self.msgs[seq]["spatialData"] = [] # Create spatial recognition array
        if name == "recognition":
...
# add in spatial data;
            ...
            # Save spatialData msg in the directory
            self.msgs[seq]["spatialData"].append(msg)
 #           self.msgs[seq][name] = msg
 #           self.msgs[seq]["jspat"] = len(msg.
       #     self.msgs[seq]["len"] = len(msg.detections)
            # print(f'Added detection seq {seq}')
