...
while True:
    ...
    if face_dets is not None:
        ...
        for det in face_dets.detections:
            jimcfg = SpatialLocationCalculatorConfig()
            jimcfg_data = SpatialLocationCalculatorConfigData()
            jimcfg_data.depthThresholds.lowerThreshold = 100
            jimcfg_data.depthThresholds.upperThreshold = 10000
            cfg = ImageManipConfig()
            correct_bb(det)
            topLeft = Point2f(det.xmin, det.ymin)
            bottomRight = Point2f(det.xmax, det.ymax)
            jimcfg_data.roi = Rect(topLeft, bottomRight)
            jimcfg.addROI(jimcfg_data)
            cfg.setCropRect(det.xmin, det.ymin, det.xmax, det.ymax)
            cfg.setResize(60, 60)
            cfg.setKeepAspectRatio(False)
            node.io['manip_cfg'].send(cfg)
            node.io['manip_img'].send(img)
            node.io['spatial_cfg'].send(jimcfg)
